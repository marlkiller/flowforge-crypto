...
import { ensureBufferSource, CryptoService, registerProvider, getProvider, type CipherProvider } from "../service";
...
async function aesImportKey(raw: Uint8Array): Promise<CryptoKey> {
  return crypto.subtle.importKey("raw", ensureBufferSource(raw), "AES-CBC", false, ["encrypt", "decrypt"]);
}
...
function makeWebCryptoProvider(
  ...
  return {
    ...
    async encrypt(keyRaw, iv, data, params) {
      const key = await crypto.subtle.importKey("raw", ensureBufferSource(keyRaw), algoName, false, ["encrypt"]);
    ...
    async decrypt(keyRaw, iv, data, params) {
      const key = await crypto.subtle.importKey("raw", ensureBufferSource(keyRaw), algoName, false, ["decrypt"]);
      const aad = params?.["aad"] as Uint8Array | undefined;
...
function makeCustomBlockProvider(
  name: string,
  ivSize: number,
  process: (key: CryptoKey, iv: Uint8Array | null, data: Uint8Array, action: "encrypt" | "decrypt") => Promise<Uint8Array>,
): CipherProvider {
  return {
    type: "cipher",
    name,
...
// Web Crypto modes
registerProvider(makeWebCryptoProvider("AES-CBC-PKCS7", "AES-CBC", 16, (iv) => ({ name: "AES-CBC", iv })));
registerProvider(makeWebCryptoProvider("AES-GCM", "AES-GCM", 16, (iv, aad) =>
  ({ name: "AES-GCM", iv, ...(aad ? { additionalData: aad } : {}) } as Algorithm)));
registerProvider(makeWebCryptoProvider("AES-CTR", "AES-CTR", 16, (iv) =>
  ({ name: "AES-CTR", counter: iv, length: 64 })));

// Custom ECB / CBC-NoPad / CFB / OFB

// ECB encrypt/decrypt with PKCS7 padding
async function ecbProcessPKCS7(key: CryptoKey, _iv: Uint8Array | null, data: Uint8Array, action: "encrypt" | "decrypt") {
  if (action === "encrypt") {
...
async function ecbProcessNoPad(key: CryptoKey, _iv: Uint8Array | null, data: Uint8Array, action: "encrypt" | "decrypt") {
...
async function cbcProcessNoPad(key: CryptoKey, iv: Uint8Array, data: Uint8Array, action: "encrypt" | "decrypt") {
...
registerProvider(makeCustomBlockProvider("AES-CFB", 16, (key, iv, data) => cfbProcess(key, iv!, data)));
registerProvider(makeCustomBlockProvider("AES-OFB", 16, (key, iv, data) => ofbProcess(key, iv!, data)));
  ...
  aes: {
    meta: {
      ...
      fields: [
        ...
        {
          id: "iv", label: "IV / Counter (Hex)", type: "text", placeholder: "All modes = 32 hex chars",
        ...
        {
          id: "aad", label: "AAD (Hex)", type: "text", placeholder: "optional hex for GCM...",
    ...
    runner: async (node, inputs) => {
      ...
      const aad = cipherMode === "GCM" ? getParamBytes(node as GraphNode, inputs, "aad", false) : undefined;
  ...
  chacha20poly1305: {
    meta: {
      ...
      fields: [
        ...
        { id: "key", label: "Key (Hex)", type: "password", placeholder: "64-char hex (256-bit)..." },
